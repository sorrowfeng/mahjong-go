...
> 整理时间：2026-08-30 ｜ 基线：316/316 测试全绿（21 套件），核心消除算子冻结
...
- combo 1-2-3-4-5 已有 `--combo-N` 样式（不同 box-shadow 强度）
- 拖动连锁消除时缺乏"震动"反馈
...
独立小改动，5 行 CSS + 3 行 JS + 1 个测试。优先做。
...
- 3 套主题（默认木韵/暗夜/清新）已落地（`js/themes.js`）
- 主题切换只换 CSS 变量，牌面图片复用

### 优化方向

- **牌背重绘**：3 套主题对应 3 张牌背（默认绿竹/暗夜金/清新蓝）
- **音效主题**：经典/现代两种音效包（合成器配置不同）
- **BGM 主题**：经典古筝/电子极简两套（仍是 Web Audio 程序化）

工作量约 100-150 行 + 资源 3 张 PNG（牌背）。
